<template>
  ...
  const getsachById = async (id) => {
    try {
      console.log("id", id);
      const response = await SachService.getOneSach(id);
      console.log("respone", response);
      console.log("response.findSach", response.findSach);
      const sachData = response.findSach; // Giả sử API trả về dữ liệu độc giả dưới dạng object
